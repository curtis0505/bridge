...
func (p *EventHandler) NewHeaderBlock(log types.Log) error {
	...
	if err != nil {
		elog.Error("NewHeaderBlock", "Unmarshal", err)
		return err
	}

	elog.Info("NewHeaderBlock",
	...
	elog.Info("NewHeaderBlock.GetBridgeWithdrawHistoryByBlockNumber", "start", eventNewHeaderBlock.Start.String(), "end", eventNewHeaderBlock.End.String(), "count", len(withdrawTxHistories))
	...
	for _, history := range withdrawTxHistories {
		...
		elog.Info("NewHeaderBlock.RegisterPendingWithdrawTx", "txHash", history.TxHash, "from", history.From, "chain", history.Chain, "cate", history.Cate)
	}
	return nil
}
