...
func (c *client) Subscribe(ctx context.Context, cb func(eventLog types.Log), addresses ...string) error {
	...
	logChan := make(chan ethertypes.Log, 5)
...
func (c *client) subscribe(ctx context.Context, addresses []string, ch chan<- ethertypes.Log, try int) (ethereum.Subscription, error) {
